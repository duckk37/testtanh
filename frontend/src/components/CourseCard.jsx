...
function CourseCard({ course }) {
  return (
    <Link to={`/courses/${course.id}`} className="block group">
      <div className="bg-white rounded-[22px] shadow-sm border border-slate-200 overflow-hidden hover:shadow-glow-hover transition-all duration-300 hover:-translate-y-1">
        <div className="aspect-video relative overflow-hidden bg-slate-100">
          ...
          <div className="absolute top-3 left-3 px-3 py-1 bg-white/90 backdrop-blur text-[11px] font-bold text-blue-700 rounded-full flex items-center gap-1 shadow-sm">
            <Star size={12} className="fill-blue-500 text-blue-500" /> Nổi bật
          </div>
          
          <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
            <div className="w-14 h-14 bg-white/90 rounded-full flex items-center justify-center text-blue-600 shadow-lg transform scale-75 group-hover:scale-100 transition-transform">
              <PlayCircle size={28} />
            </div>
          </div>
        </div>
        ...
        <div className="p-5">
          <h3 className="font-bold text-[17px] text-slate-900 mb-2 line-clamp-2 leading-snug group-hover:text-blue-600 transition-colors">
            {course.title}
          </h3>
