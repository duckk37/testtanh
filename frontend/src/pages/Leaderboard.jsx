  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 font-sans pb-20">
      ...
      <div className="max-w-3xl mx-auto px-4 -mt-16 relative z-10">
          ...
          <div className="flex items-end justify-center gap-2 md:gap-6 mb-8 px-2">
            ...
            {top3[1] && (
              <div className="flex flex-col items-center w-1/3">
                <div className="relative mb-2">
                  <div className="w-16 h-16 md:w-20 md:h-20 bg-slate-200 rounded-full border-4 border-white flex items-center justify-center text-xl font-bold text-slate-500 dark:text-slate-400 shadow-md">
                    {top3[1].username.charAt(0).toUpperCase()}
                  </div>
                  <div className="absolute -bottom-2 -right-2 bg-slate-300 text-slate-700 dark:text-slate-200 w-8 h-8 rounded-full flex items-center justify-center font-bold border-2 border-white shadow-sm">2</div>
                </div>
                ...
                <div className="w-full h-24 md:h-32 bg-gradient-to-t from-slate-200 to-slate-100 rounded-t-xl border border-slate-200 dark:border-slate-700 border-b-0 shadow-inner"></div>
              </div>
            )}
            ...
            {top3[0] && (
              <div className="flex flex-col items-center w-1/3 -mt-8 relative z-10">
                ...
                <div className="text-center mb-3">
                  <p className="font-bold text-slate-900 dark:text-slate-100 text-base md:text-lg truncate w-28">{top3[0].username}</p>
        ...
        {/* Other Ranks List */}
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-soft dark:shadow-none border border-slate-100 dark:border-slate-700/50 overflow-hidden">
          {others.length > 0 ? others.map((user, index) => (
            <div key={user.id} className="flex items-center p-4 border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50 dark:bg-slate-900 transition-colors">
              <div className="w-8 font-bold text-slate-400 text-center mr-4">{index + 4}</div>
              <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-600 dark:text-slate-300 mr-4">
